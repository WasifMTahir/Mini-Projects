...
func dictionary(codes []int) [][]int {
    ...
    for i:=2; i<len(codes); i++ {
        fmt.Println(dict[codes[i]])
        if codes[i] < marker {
            dict[i+marker] = make([]int, 1)
            dict[i+marker][0] = dict[codes[i]][0]
        } else {
            dict[i+marker] = make([]int, 1)
            dict[i+marker][0] = dict[i+marker-1][0]
        }
        fmt.Println(marker)
    }
...
func parallel_dictionary(codes []int) []chan []int {
    dict := make([]chan[]int, 4096)
    return dict
}
...
func main() {
    ...
    for i, ch := range parallel_dictionary(part5_gif_codes) {
        if !reflect.DeepEqual(part5_gif_dict[i], <-ch) {
