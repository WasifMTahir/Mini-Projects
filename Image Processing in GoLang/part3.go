...
func parallel_decompress(codes []int, dict[][]int, size chan<- int, slice <-chan []int) {
    ...
    //parallel_helper(codes, dict, size, slice)
    out1 := parallel_other(codes, dict, slice)
    size<- out1
}
/*
func parallel_helper(codes []int, dict [][]int, size chan<- int, slice <-chan []int) {
    size<-128
    outArr := make([]int, 128)
    if len(codes) > 16 {
        mid := len(codes)/2
        ...
        go func() {
            parallel_helper(codes[:mid], dict, size, <-slice[:(mid*3)])
            done1<- true
        }()
        parallel_helper(codes[mid:], dict, size, slice[(mid*3):])
        <-done1
...
*/
func parallel_other(codes []int, dict [][]int, slice <-chan []int) int {
    size := 0
    for i:=0; i<len(codes); i++ {
        out := dict[codes[i]]
        size = size + len(out)
