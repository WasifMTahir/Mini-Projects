...
splitLine (x:xs) n
	| (length x) > n = ([], (x:xs))
  | otherwise = ([x] ++ fst(splitLine xs (n-(length x)-1)), snd(splitLine xs (n-(length x)-1)))
...
-- Function definition here
blankInsertions :: Int -> [String] -> [[String]]
blankInsertions n [x] = [[x]]
blankInsertions n (x:xs)
                | length [x] < 3 = [[x]]
                | n == 1 = [[x ++ " " ++ xs]]
                | otherwise = blankInsertions n-1 (xs)
...
hypCost = 1.0
-----------------------------------------------
totalBlanks :: [String] -> Double
totalBlanks [] = 0
totalBlanks (x:xs)
  | x == " " = 1 + totalBlanks(xs)
  | otherwise = 0 + totalBlanks(xs)

firstBlank :: [String] -> Double
firstBlank [] = 0
firstBlank (x:xs)
  | xs == " " = 0
  | otherwise = 1 + firstBlank(xs)

lastBlank :: [String] -> Double
lastBlank [] = 0
lastBlank (x:xs) = firstBlank (reverse (x:xs))
...
-- Function definition here
blankCost * 2.0 + blankProxCost * (5 - average(firstBlank(x:xs), 0, lastBlank(x:xs))) + blankUnevenCost * variance(firstBlank(x:xs), 0, lastBlank(x:xs))
